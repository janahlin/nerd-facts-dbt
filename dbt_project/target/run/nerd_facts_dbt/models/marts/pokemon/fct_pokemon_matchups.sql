  ...
  create  table "nerd_facts"."public"."fct_pokemon_matchups__dbt_tmp"
...
WITH type_pairs AS (
    ...
    FROM "nerd_facts"."public"."dim_pokemon_types" a
    CROSS JOIN "nerd_facts"."public"."dim_pokemon_types" d
    ...
    SELECT
        ...
        CASE
            ...
            WHEN tp.defending_type IN (
                ...
                FROM "nerd_facts"."public"."dim_pokemon_types" 
...
SELECT
    -- Primary key
    md5(cast(coalesce(cast(te.attacking_type as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(te.defending_type as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) AS matchup_id,
...
JOIN "nerd_facts"."public"."dim_pokemon_types" at ON te.attacking_type = at.type_name  -- Join for attacking type metadata
JOIN "nerd_facts"."public"."dim_pokemon_types" dt ON te.defending_type = dt.type_name  -- Join for defending type metadata
